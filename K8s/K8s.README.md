...
```
En Windows te crea el archivo en UTF-16 así que lo necesitas en UTF-8
```
kubectl create deployment client-gateway --image=us-south1-docker.pkg.dev/tienda-microservicios-431002/image-registry/client-gateway --dry-run=client -o yaml | Out-File -FilePath deployment.yml -Encoding utf8
...
```
Para UTF8 en Windows
```
$tempFilePath = '<Path donde lo quieren>\temp-service.yml'
$finalFilePath = '<Path donde lo quieren>\service.yml'

kubectl create service <nodeport|clusterip> <nombre>> --tcp=<8888> --dry-run=client -o yaml | Out-File -FilePath $tempFilePath -Encoding utf8
